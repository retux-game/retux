...
SECOND_POINTS = 500 if HELL else 100
COIN_POINTS = 100
...
class Player(xsge_physics.Collider):
    ...
    name = "Super-Penguin Tux" if GOD else "Heretic Tux" if HELL else "Tux"
...
class KeyboardMenu(Menu):
    ...
    def event_choose(self):
        ...
        if self.choice == 0:
            ...
            k = wait_key(text)
            if k is not None:
                bind_key(action_key[self.page], k)
                play_sound(confirm_sound)
            else:
                play_sound(cancel_sound)
            self.__class__.create_page(default=self.choice, page=self.page)
            ...
            k = wait_key(text)
            if k is not None:
                bind_key(sneak_key[self.page], k)
                play_sound(confirm_sound)
            else:
                play_sound(cancel_sound)
            self.__class__.create_page(default=self.choice, page=self.page)
...
class JoystickMenu(Menu):
    ...
    def event_choose(self):
        ...
        if self.choice == 0:
            ...
            js = wait_js(text)
            if js is not None:
                bind_js(action_js[self.page], js)
                play_sound(confirm_sound)
            else:
                play_sound(cancel_sound)
            self.__class__.create_page(default=self.choice, page=self.page)
